classdef networkTrafficVoIP < comm.internal.ConfigBase & handle
    ...
    function [dt, packetSize, varargout] = generate(obj, varargin)
            ...
            if obj.NextInvokeTime <= 0
                ...
                if obj.GeneratePacket
                    varargout{1} = obj.AppData(1:packetSize, 1);
...
methods(Access = private)
    function lrand = laplaceRandomNumber(obj)
        ...
        %
        % LRAND = laplaceRandomNumber(OBJ) Return a random number generated drawn
        % from the Laplace distribution
        %
        % LRAND is the random number generated drawn from the Laplace distribution

        % Initialize lrand to address code generation error.
        lrand = obj.LaplaceMu;
        generateLrand = true;
        while generateLrand
            u = rand;
