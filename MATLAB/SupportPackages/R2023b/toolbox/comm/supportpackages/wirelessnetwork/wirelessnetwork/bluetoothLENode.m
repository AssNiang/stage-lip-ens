...
%   pushChannelData        - Push data from channel to reception buffer of Bluetooth LE node
%   channelInvokeDecision  - Return flag to indicate whether channel has to
%                            be applied on incoming signal at Bluetooth LE node
...
properties (SetAccess = private)
    ...
    %object or an array of objects of type <a
...
properties (Access = private)
    ...
    %pConnectedNodeIDs Vector of node IDs which are associated with this
    %node
    pConnectedNodeIDs
...
methods
    ...
    function obj = bluetoothLENode(role, varargin)
        ...
        obj.PHYReceiver = ble.internal.phyReceiver;
...
methods (Access = protected)
    function flag = isInactiveProperty(obj, prop)
        ...
        switch prop
            ...
            % peripheral roles
            case {"ConnectionConfig", "CISConfig"}
...
methods
    function addTrafficSource(obj, trafficSource, varargin)
        ...
        %   to the node for pumping traffic to the specified connected isochronous stream (CIS)
        %   connection, CISConfig, in the destination node, DestinationNode. To enable
        %   this syntax, set the <a href="matlab:help('bluetoothLENode.Role')">Role</a> property of <a href="matlab:help('bluetoothLENode')">bluetoothLENode</a> object
        %   to "central" or "peripheral".
        %
        %   The "CISConfig" input argument specifies the CIS connection in the destination
        %   node. Specify this input as an object of type <a
        %   href="matlab:help('bluetoothLECISConfig')">bluetoothLECISConfig</a>.
    ...
    function nextInvokeTime = runNode(obj, currentTime)
        ...
        coder.internal.error("bluetooth:bluetoothLENode:DeprecatedFunction", "runNode");
    ...
    function pushChannelData(obj, packet)
        ...
        coder.internal.error("bluetooth:bluetoothLENode:DeprecatedFunction", "pushChannelData");
    ...
    function [flag, rxInfo] = channelInvokeDecision(obj, packet)
        ...
        %incoming signal at Bluetooth LE node
        %
        ...
        coder.internal.error("bluetooth:bluetoothLENode:DeprecatedFunction", "channelInvokeDecision");
...
methods (Hidden)
    function addConnection(obj, connectionConfig, varargin)
        ...
        obj.pConnectedNodeIDs(obj.NumConnections) = destinationID;

        % Update the number of CIS connections and CIS configuration
        if nargin == 3
            cisConfig = varargin{1};
            ...
            switch value
                case "DestinationNode"
                    ...
                    if isa(destinationNode, "bluetoothLENode")
                        connectionIndex = find(destinationNode.ID==obj.pConnectedNodeIDs);
                    else
                        connectionIndex = find(strcmpi(destinationNode, obj.pConnectedNodes));
                    end
                    coder.internal.errorIf(isempty(connectionIndex), ...
                        "bluetooth:bluetoothLENode:InvalidDestinationNode", connectedNodes(1,:));
                    coder.internal.errorIf(numel(connectionIndex)>1,"bluetooth.bluetoothLENode:SameDestinationName");
                    upperLayerData.ConnectionIndex = connectionIndex;
                case "CISConfig"
                    validateattributes(varargin{i+1}, {'bluetoothLECISConfig'}, ...
                        {'row'}, mfilename, "CISConfig");
                    cisConfig = varargin{i+1};
                    upperLayerData.CISID = cisConfig.CISID;
                    coder.internal.errorIf(trafficSource.PacketSize > cisConfig.MaxPDU, ...
                        "bluetooth:bluetoothLENode:InvalidPacketLength", obj.Role, "1", num2str(cisConfig.MaxPDU));
                case "SourceAddress"
            ...
            case {"central", "peripheral"}
                ...
                if ~isempty(upperLayerData.CISID)
                    pushUpperLayerPDU(obj.pLinkLayer, connectionIndex, ...
                        upperLayerData.Data, obj.pCurrentTimeInMicroseconds, upperLayerData.CISID);
