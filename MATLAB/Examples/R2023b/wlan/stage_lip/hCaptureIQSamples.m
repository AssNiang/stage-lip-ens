        ...
        function obj = hCaptureIQSamples(node,varargin)
            ...
            if isa(node,"helperCoexNode")
                ...
                if ~isempty(node.WLANDevice.DeviceName)
                    obj.CenterFrequency = [obj.CenterFrequency node.WLANDevice.Frequencies];
                    obj.Bandwidth = [obj.Bandwidth node.WLANDevice.DeviceConfig.ChannelBandwidth];
                    obj.pExpectedSampleRate = [obj.pExpectedSampleRate node.WLANDevice.DeviceConfig.ChannelBandwidth];
                    obj.pDeviceNames(countDevice) = strjoin([nodeName "WLAN" num2str(node.WLANDevice.Frequencies/1e6) "MHz"],"_");
            ...
            elseif isa(node,"wlanNode")
                obj.CenterFrequency = node.Frequencies;
                obj.Bandwidth = [node.DeviceConfig.ChannelBandwidth];
                for idx=1:numel(obj.CenterFrequency)
                    obj.pDeviceNames(idx) = strjoin([nodeName num2str(node.Frequencies(idx)/1e6) "MHz"],"_");
                end
    ...
    methods (Access=private)
        function captureSignals(obj,node,networkSimulator,duration)
            ...
            if currentTime==0
                ...
                elseif isa(node,"wlanNode")
                    ...
                    captureIQSamples(obj,[node.PHYRx.Interference],duration,[node.DeviceConfig.NumTransmitAntennas]);
        ...
        function initializeIQData(obj,node,endTime,duration)
            ...
            elseif isa(node,"wlanNode") % WLAN node
                ...
                numRxAnts = [node.DeviceConfig.NumTransmitAntennas]; % Tx and Rx antenna count is same
                obj.pSize = zeros(numel(node.Frequencies),1);
                obj.pStartTime = zeros(numel(node.Frequencies),1);
        ...
        function validateWLANNode(obj,node)
            ...
            validateattributes(obj.CenterFrequency,"double",{'vector','numel',numel(node.DeviceConfig)},mfilename,"CenterFrequency");
            validateattributes(obj.Bandwidth,"double",{'vector','numel',numel(node.DeviceConfig)},mfilename,"Bandwidth");
            ...
            if numel(obj.CenterFrequency)>1
                ...
                centerFreqBandNode = round(node.Frequencies/1e6);
