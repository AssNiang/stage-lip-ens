...
%   Copyright 2022-2023 The MathWorks, Inc.
    ...
    methods
        function obj = hSLSTGaxMultiFrequencySystemChannel(nodes,varargin)
            ...
            for idx = 1:numNodes
                assert((nodes(idx).PHYAbstractionMethod=="none") == obj.UseFullPHY ,'All nodes must be configured to use the same PHY abstraction method')

                for ifcIdx = 1:numel(nodes(idx).DeviceConfig)
                    nodeFreqs(end+1) = wlanChannelFrequency(nodes(idx).DeviceConfig(ifcIdx).BandAndChannel(2),nodes(idx).DeviceConfig(ifcIdx).BandAndChannel(1)); %#ok<AGROW>
                    nodeBWs(end+1) = nodes(idx).DeviceConfig(ifcIdx).ChannelBandwidth; %#ok<AGROW>
                end
            end
            ...
            for i = 1:numel(uniqueFreqs)
                ...
                if ~all(bwForFreq==bwForFreq(1))
                    error('All nodes using the same bandwidth must use the same frequency')
                end
            ...
            for i = 1:numel(uniqueFreqs)
                for n = 1:numNodes
                    activeFreq = nodes(n).Frequencies==uniqueFreqs(i);
                    if ~any(activeFreq)
                        continue
                    end
                    numAnts(n,i) = nodes(n).DeviceConfig(activeFreq).NumTransmitAntennas;
                end
            end
